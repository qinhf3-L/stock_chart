...
class TrendKLineWidget(QWidget):
    init_completed = False

    def __init__(self, ts_code=None):
        """Constructor"""
        super(TrendKLineWidget, self).__init__()
        self.stock = None
            ...
            stock = pd.merge(
                pd.read_sql(
                    ...
                    con=engine())[["ts_code", "open", "close", "low", "high", "pct_chg", "trade_date", "vol", "dema_3",
                                   ...
                                   "macd_histogram"]],
                pd.read_sql(
                    sql="select * from trend_stock_pool_tab where ts_code='%s' and delete_status=0" % ts_code,
                    con=engine())[["ts_code", "name", "count", "level"]],
                on="ts_code")
    ...
    def set_stock_info(self):
        ...
        self.chg_label.setText("涨跌幅:     " + str(latest_data.get("pct_chg")),
                               size='12pt', color=color)
        self.weight_label.setText("推荐力度:     " + str(latest_data.get("count")),
                                  size='12pt', color=color)
