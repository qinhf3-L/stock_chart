...
from qtpy.QtWidgets import QWidget, QTreeWidget, QVBoxLayout, QHBoxLayout
...
class StockPoolWidget(QWidget):
    ...
    def __init__(self, parent=None):
        super(StockPoolWidget, self).__init__(parent)
    ...
    def _on_tree_click(self, item):
        if item.childCount() == 0:
            self.pw.set_stock_code(item.text(1))
            # data = self.stocks_daily[self.stocks_daily["ts_code"] == item.text(1)]
            # self.pw.loadData(data)
            # lastest_data = data.iloc[-1].to_dict()
            # self.setWindowTitle("%s %s"%(lastest_data.get("ts_code"), lastest_data.get("name")))
    ...
    def initUI(self):
        ...
        self.pw = TabWidget(self.current_ts_code)
    ...
    def _refreshTree(self):
        ...
        self.tree.addTopLevelItem(item)
        short_stocks = pd.read_sql(sql="select * from short_stock_pool_tab where delete_status = 0 order by count desc, weight desc", con=engine())
